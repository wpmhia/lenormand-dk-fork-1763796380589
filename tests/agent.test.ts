...
describe('pipToDeadline', () => {
  ...
  it('should return "by Thursday evening" or "by Friday evening"', () => {
    ...
    expect(
      result.text === 'by Thursday evening' || result.text === 'by Friday evening'
    ).toBe(true)
  })
  ...
  it('should cap at 14 days for long pips', () => {
    ...
    const diffDays = Math.floor((result.date.getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
...
describe('makeTask', () => {
  it('should generate task for Ring card', () => {
    const cards: LenormandCard[] = [dogCard, coffinCard, { id: 25, name: 'Ring' }]
    const task = makeTask('verdict', cards)
    expect(task.toLowerCase()).toContain('sign')
  })

  it('should generate task for Letter card', () => {
    const cards: LenormandCard[] = [dogCard, coffinCard, { id: 27, name: 'Letter' }]
    const task = makeTask('verdict', cards)
  ...
  it('should generate task for Crossroad card', () => {
    const cards: LenormandCard[] = [dogCard, coffinCard, { id: 22, name: 'Crossroad' }]
    const task = makeTask('verdict', cards)
...
describe('SPREAD_RULES', () => {
  ...
  it('should have valid template types', () => {
    const validTemplates = ['1-card', '3-card', '5-card', '7-card', '9-card', '36-card']
    Object.values(SPREAD_RULES).forEach(rule => {
