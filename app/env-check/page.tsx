...
export default function EnvCheckPage() {
  ...
  const missingCount = ENV_VARIABLES.filter(
    (env) => !envStatus[env.name],
  ).length;
          ...
          <>
            <Alert className="bg-destructive/10 text-destructive border-destructive mb-3 p-3">
              <div className="flex items-center gap-2">
                ...
                <AlertDescription>
                  <strong>
                    {missingCount} environment variable
                    {missingCount > 1 ? "s are" : " is"} missing.
                  </strong>
                </AlertDescription>
              </div>
            </Alert>
            <p className="mb-6 text-sm text-muted-foreground">
              Please configure{" "}
              {missingCount > 1 ? "these variables" : "this variable"} to ensure
        ...
        <Card className="mt-8">
          ...
          <CardContent className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Click the buttons above to open debug endpoints in new tabs. These
              will show you exactly what&apos;s happening in your deployed
              environment.
            </p>
          </CardContent>
        </Card>
