...
const getGrandTableauPosition = (
  index: number,
): { row: number; col: number; label: string; stripInfo: string } => {
  ...
  const getStripInfo = (
    ...
    const isAdjacent =
      (Math.abs(row - sigRow) === 1 && col === sigCol) ||
  ...
  const renderLayout = () => {
    if (reading.layoutType === 1) {
      ...
      const significatorIndex = reading.cards.findIndex(
        (rc) => rc.id === 29 || rc.id === 28 || rc.id === 13 || rc.id === 18,
      );
      const significatorCard =
        significatorIndex !== -1
          ? getCardById(allCards, reading.cards[significatorIndex].id)
          : null;
      const sigRow = Math.floor(significatorIndex / 9);
      const sigCol = significatorIndex % 9;
      ...
      const getCardStripClass = (index: number): string => {
        ...
        if (row === sigRow) {
          if (col < sigCol)
            return "bg-blue-100 dark:bg-blue-950 border-blue-400";
          if (col > sigCol)
            return "bg-green-100 dark:bg-green-950 border-green-400";
        }

        // Strip B: Same column (mind)
        if (col === sigCol) {
          if (row < sigRow)
            return "bg-purple-100 dark:bg-purple-950 border-purple-400";
          if (row > sigRow)
            return "bg-indigo-100 dark:bg-indigo-950 border-indigo-400";
        }
      ...
      return (
        <div className="space-y-6">
            ...
            <div className="text-sm text-muted-foreground">
              {significatorCard ? (
                <div className="space-y-1">
                  <p>
                    <strong>Significator: {significatorCard.name}</strong>{" "}
                    (Position {significatorIndex + 1})
                  </p>
                  <p className="mt-2 text-xs">
                    Colors show the Five Strips: Blue/Green = Row (narrative),
                    Purple/Indigo = Column (mind), Golden = Cross (pivot), Amber
                    = Corners (fate)
                  </p>
                </div>
              ) : (
                <span>
                  No significator (Woman/Man/Child/Dog) found in spread
                </span>
              )}
            </div>
          </div>
          ...
          {/* 4x9 Grid */}
          <div
            className="mx-auto grid max-w-6xl gap-sm rounded-lg border-2 border-primary/30 bg-card/10 p-md"
            ...
            {reading.cards.map((readingCard, index) => {
              ...
              return (
                <AnimatedCard
                  key={index}
                  delay={index * 0.05}
                  className={`flex flex-col items-center space-y-sm rounded-lg border-2 transition-all ${stripClass} ${isSignificator ? "ring-2 ring-primary" : ""}`}
                >
          ...
          <div className="space-y-md rounded-lg border border-border bg-muted/50 p-md text-sm text-muted-foreground">
            ...
            <ol className="list-inside list-decimal space-y-sm text-xs">
              <li>
                <strong>Strip A (Row):</strong> Read the nine cards in the
                significator&apos;s row (left→right) as the narrative of the
                moment
              </li>
              <li>
                <strong>Strip B (Column):</strong> Read the four cards in the
                significator&apos;s column (top→bottom) to understand what
                weighs on the mind
              </li>
              <li>
                <strong>Strip C (Cross):</strong> The four adjacent cards form
                the immediate pivot—the crossing point
              </li>
              <li>
                <strong>Strip D (Corners):</strong> The four corners show
                fate&apos;s intention and the overall frame
              </li>
              <li>
                <strong>Strip E (Knights):</strong> Optional—leap one card in
                eight directions for hidden influences
              </li>
            </ol>
          </div>
        </div>
      ...
      return (
        <div
          className={`mx-auto grid gap-lg overflow-visible ${
            reading.layoutType === 3
              ? "max-w-4xl grid-cols-1 sm:grid-cols-3"
              : reading.layoutType === 5
                ? "max-w-6xl grid-cols-1 sm:grid-cols-3 md:grid-cols-5"
                : reading.layoutType === 7
                  ? "max-w-6xl grid-cols-1 sm:grid-cols-3 md:grid-cols-7"
                  : reading.layoutType === 9
                    ? "max-w-7xl grid-cols-1 sm:grid-cols-3 md:grid-cols-3"
    ...
    <div className="space-y-2xl">
        ...
        <div className="animate-in fade-in slide-in-from-top-8 duration-500">
          <div className="rounded-lg border border-border bg-card p-xl text-center shadow-elevation-2">
            ...
            {reading.question && reading.question !== reading.title && (
              <p className="mt-lg text-lg italic text-muted-foreground">
            ...
            <div className="mt-xl flex flex-wrap items-center justify-center gap-lg text-sm text-muted-foreground">
              ...
              {countdown && (
                <div
                  className={`flex items-center gap-2 ${countdown.isExpired ? "text-muted-foreground/50 line-through" : "text-primary"}`}
                >
                  <Clock className="h-4 w-4" />
                  <span
                    className={countdown.isExpired ? "italic" : "font-medium"}
                  >
                    {countdown.text}
                  </span>
                </div>
          ...
          {countdown?.isExpired && (
            <div className="pointer-events-none absolute inset-0 rounded-xl bg-muted/20 opacity-40" />
          )}
        </div>
      )}
      ...
      {selectedCard && (
        <CardModal
          ...
          position={
            reading.cards.find((c) => c.id === selectedCard.id)?.position
          }
        />
      )}
