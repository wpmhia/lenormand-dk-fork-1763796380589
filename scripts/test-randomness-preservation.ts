...
function runRandomnessTest() {
  ...
  // Test 1: Same cards, different questions
   ...
   const sameCards = testCases[0] as any;
   
   if (sameCards.questions) {
     sameCards.questions.forEach((question: string, index: number) => {
    const category = categorizeQuestion(question);
    const cacheKey = generateCacheKey(sameCards.cards, 'sentence-3', category);
    const interpretation = generateUniqueInterpretation(sameCards.cards, 'sentence-3', category, question);
   ...
   const sameQuestion = testCases[1] as any;
   const categoryTest2 = sameQuestion.question ? categorizeQuestion(sameQuestion.question) : 'GENERAL';
   
   if (sameQuestion.question && sameQuestion.cards) {
     ...
     mockTimes.forEach((mockDateStr, index) => {
       ...
       const cacheKey = generateCacheKey(sameQuestion.cards, 'sentence-3', categoryTest2);
       const interpretation = generateUniqueInterpretation(sameQuestion.cards, 'sentence-3', categoryTest2, sameQuestion.question);
  ...
  const orderedTests = [
   ...
   orderedTests.forEach(test => {
     ...
     const cacheKey = generateCacheKey(testCards, 'sentence-3', categoryTest2);
