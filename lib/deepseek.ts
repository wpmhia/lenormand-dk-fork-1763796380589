...
export async function getAIReading(request: AIReadingRequest): Promise<AIReadingResponse | null> {
  ...
  try {
    ...
    // Simple token allocation - keep it low
    let maxTokens = 400
    if (cards.length >= 36) maxTokens = 600
    else if (cards.length >= 9) maxTokens = 500
    else if (cards.length >= 7) maxTokens = 450
    else if (cards.length >= 5) maxTokens = 400
