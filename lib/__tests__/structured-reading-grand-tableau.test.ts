...
describe("Grand Tableau positional evidence", () => {
  it("accepts valid position evidence references", () => {
    ...
    const reading = getStructuredReadingSchema("grand-tableau").parse({
      ...
      evidence: [{ claimId: "position-3", pair: "tableau positions", evidenceIds: ["position-3"], implication: "Position three contributes context to the reading." }],
      prediction: { development: "The situation develops through the supplied tableau evidence.", evidenceIds: ["position-3"], claimIds: ["question-predicate"], timing: "Not clearly shown by these cards.", watchFor: null, practicalAction: null },
