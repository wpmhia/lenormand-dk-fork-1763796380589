...
export const QuestionFrameSchema = z.object({
  ...
  mode: z.enum(["forecast", "retrospective_event"]),
...
export async function parseQuestionFrame(question: string, model: LanguageModel, signal?: AbortSignal): Promise<QuestionFrame> {
  const result = await generateText({
    ...
    abortSignal: signal,
    maxOutputTokens: 240,
    system: "Parse the user's question only. Return the requested structured object with one canonical mode: forecast or retrospective_event. Detect the exact predicate without relying on fixed keywords. Do not infer an answer, card meaning, polarity, or outside facts. Preserve ambiguity.",
