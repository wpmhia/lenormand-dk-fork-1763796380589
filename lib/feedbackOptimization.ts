...
export async function recordFeedback(
  ...
  try {
    ...
    if (cards && cards.length > 0 && spreadId) {
      console.log(
        `[Model Learning] Recorded ${isHelpful ? "positive" : "negative"} feedback for ${cards.length} cards in ${spreadId}`,
      );
    }
 ...
 */
async function updatePromptVariantStats(
  ...
  try {
    ...
    const helpfulnessRate =
      totalCount > 0 ? (newHelpfulCount / totalCount) * 100 : 0;
 ...
 */
async function updateFeedbackPatterns(
  spreadId: string,
  isHelpful: boolean,
): Promise<void> {
  try {
    ...
    if (existingPattern) {
      const newHelpfulCount =
        existingPattern.helpfulCount + (isHelpful ? 1 : 0);
...
export async function generateMetricsReport(
  ...
  try {
    ...
    const prevStartDate = new Date(
      startDate.getTime() - daysBetween * 1000 * 60 * 60 * 24,
    );
    const prevMetrics = await getSpreadMetrics(
      spreadId,
      prevStartDate,
      startDate,
    );
...
export async function getOptimizationStatus(): Promise<{
  ...
  try {
    ...
    for (const fb of allFeedback) {
      ...
      const current = spreadMetrics.get(spreadIdKey) || {
        helpful: 0,
        total: 0,
      };
      current.total += 1;
    ...
    const spreads = Array.from(spreadMetrics.entries()).map(
      ([spreadId, metrics]) => ({
        spreadId,
        helpfulnessRate:
          metrics.total > 0 ? (metrics.helpful / metrics.total) * 100 : 0,
    ...
    return {
      ...
      systemHelpfulnessRate:
        systemTotal > 0 ? (systemHelpful / systemTotal) * 100 : 0,
    };
  } catch (error) {
