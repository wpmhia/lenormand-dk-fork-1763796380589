# SEO Resources & Tools - Quick Reference

## Essential Google Tools (Free)

### 1. Google Search Console

**URL:** https://search.google.com/search-console/
**Purpose:** Monitor search performance, submit sitemap, fix errors
**Key Actions:**

- Submit sitemap.xml
- Monitor keyword rankings
- Check indexing status
- Fix crawl errors
- View search analytics

### 2. Google Analytics 4

**URL:** https://analytics.google.com/
**Purpose:** Track user behavior and conversions
**Key Metrics:**

- Organic traffic source
- User engagement (time on site, bounce rate)
- Conversion tracking (reading completions)
- Device breakdown (mobile vs desktop)

### 3. Google Pagespeed Insights

**URL:** https://pagespeed.web.dev/
**Purpose:** Analyze Core Web Vitals and page speed
**Key Metrics:**

- LCP (Largest Contentful Paint)
- FID (First Input Delay)
- CLS (Cumulative Layout Shift)
- Overall score (target: 90+)

### 4. Google Lighthouse

**Access:** Chrome DevTools → Lighthouse tab
**Purpose:** Comprehensive page audit
**Tests:**

- Performance
- Accessibility
- Best practices
- SEO
- PWA readiness

### 5. Google Structured Data Testing Tool

**URL:** https://schema.org/validator
**Purpose:** Validate JSON-LD schemas
**Test:** Paste your JSON-LD and verify syntax

---

## Free SEO Tools (Third-Party)

### 1. Ubersuggest (Freemium)

**URL:** https://ubersuggest.com/
**Use For:**

- Keyword research
- Competitor analysis
- Content ideas
- Backlink analysis

### 2. Moz Local Listing Audit (Free)

**URL:** https://moz.com/local/listing-audit
**Use For:**

- Local SEO check
- Citation audit
- Listing consistency

### 3. Yoast SEO Checker (Free)

**URL:** https://www.yoast.com/
**Use For:**

- Keyword density analysis
- Readability check
- Meta description preview

### 4. AnswerThePublic (Freemium)

**URL:** https://answerthepublic.com/
**Use For:**

- Question mining
- Long-tail keywords
- Content ideas

### 5. Keywordtool.io (Freemium)

**URL:** https://keywordtool.io/
**Use For:**

- Long-tail keyword research
- Google Suggest keywords
- YouTube keyword research

---

## Content Creation Tools

### 1. Grammarly

**URL:** https://www.grammarly.com/
**Use For:**

- Writing quality
- Grammar checking
- Plagiarism detection

### 2. Hemingway Editor

**URL:** https://www.hemingwayapp.com/
**Use For:**

- Readability improvement
- Clarity enhancement
- Sentence structure

### 3. Copyscape

**URL:** https://www.copyscape.com/
**Use For:**

- Plagiarism detection
- Duplicate content check

---

## Backlink & Authority Tools

### 1. Google Search Console

**Inbound links:** Search Console → Links → Top linking sites

### 2. Semrush (Freemium)

**URL:** https://www.semrush.com/
**Use For:**

- Backlink analysis
- Competitor backlinks
- Authority score

### 3. Ahrefs (Freemium)

**URL:** https://ahrefs.com/
**Use For:**

- Backlink analysis
- Referring domains
- Link opportunities

### 4. Moz Domain Authority Checker

**URL:** https://moz.com/domain-analysis
**Use For:**

- Domain authority score
- Comparing competitors

---

## Competitor Analysis Tools

### 1. SEMrush

**Use For:**

- Competitor keywords
- Competitor backlinks
- Competitor strategies

### 2. Ahrefs

**Use For:**

- Site explorer
- Keyword gap analysis
- Competitor content

### 3. SimilarWeb

**URL:** https://www.similarweb.com/
**Use For:**

- Traffic analysis
- Traffic sources
- Competitor benchmarking

---

## Rank Tracking Tools

### 1. Rank Tracker (Freemium)

**URL:** https://www.ranktracker.com/
**Use For:**

- Daily rank tracking
- Rank history
- Competitor comparison

### 2. AccuRanker

**URL:** https://www.accuranker.com/
**Use For:**

- Accurate daily tracking
- Batch reports
- Integration with other tools

---

## Social Media & Content Tools

### 1. Buffer

**URL:** https://buffer.com/
**Use For:**

- Social media scheduling
- Performance tracking
- Team collaboration

### 2. Canva

**URL:** https://www.canva.com/
**Use For:**

- Image creation
- Social graphics
- Blog headers

---

## Email & Newsletter Tools

### 1. Mailchimp (Free tier)

**URL:** https://mailchimp.com/
**Use For:**

- Newsletter signup forms
- Email marketing
- Audience building

---

## Local & Citation Tools

### 1. Google My Business

**URL:** https://business.google.com/
**Use For:**

- Local SEO
- Business info
- Reviews management

### 2. Yelp for Business

**URL:** https://biz.yelp.com/
**Use For:**

- Business listing
- Review management

---

## SEO Monitoring Scripts & Plugins

### 1. Chrome SEO Extension

**Name:** SEO QuakeColor
**Use For:**

- Quick SEO metrics
- Page analysis on-the-fly

### 2. Lighthouse Chrome Extension

**Use For:**

- One-click page audits
- Real-time Core Web Vitals

### 3. Wappalyzer

**URL:** https://www.wappalyzer.com/
**Use For:**

- Technology detection
- Competitor tech stack

---

## SEO Learning Resources

### 1. Google Search Central

**URL:** https://developers.google.com/search
**Content:**

- Official SEO guidelines
- Core Web Vitals guide
- Best practices

### 2. MOZ SEO Learning Hub

**URL:** https://moz.com/learn/seo
**Content:**

- Beginner's guide
- Advanced topics
- Industry updates

### 3. Search Engine Journal

**URL:** https://www.searchenginejournal.com/
**Content:**

- Daily news
- Tutorials
- Case studies

### 4. Backlinko

**URL:** https://backlinko.com/
**Content:**

- In-depth guides
- Research studies
- Best practices

---

## Reporting & Analytics Tools

### 1. Data Studio (Free)

**URL:** https://datastudio.google.com/
**Use For:**

- Custom dashboards
- Report automation
- Data visualization

### 2. Google Sheets

**URL:** https://sheets.google.com/
**Use For:**

- Rank tracking spreadsheet
- Keyword research tracking
- Simple reporting

---

## SEO Audit Tools

### 1. Sitechecker (Freemium)

**URL:** https://sitechecker.pro/
**Use For:**

- Full site audit
- Error detection
- On-page optimization

### 2. WebPageTest

**URL:** https://www.webpagetest.org/
**Use For:**

- Detailed performance analysis
- Video playback
- Speed comparison

---

## Quick Reference: Weekly SEO Tasks

### Daily (5 min)

- [ ] Check Google Search Console alerts
- [ ] Monitor Core Web Vitals

### Weekly (30 min)

- [ ] Review ranking positions
- [ ] Check indexing status
- [ ] Analyze traffic from Search Console

### Monthly (2 hours)

- [ ] Deep dive competitor analysis
- [ ] Content performance review
- [ ] Keyword ranking analysis
- [ ] Backlink check

### Quarterly (half day)

- [ ] Full site audit
- [ ] Strategy review
- [ ] Content calendar planning
- [ ] Link building outreach

---

## SEO Metrics Cheat Sheet

```
On-Page SEO:
- Title length: 50-60 characters
- Meta description: 150-160 characters
- URL length: <75 characters
- H1 count: 1 per page
- Keyword density: 1-2%

Technical SEO:
- Page load time: <3 seconds
- Mobile score: 90+
- Desktop score: 90+
- Core Web Vitals: All "Good"

Authority:
- Domain Authority: 30+
- Page Authority: 20+
- Backlinks: 50+ quality links
- Referring domains: 20+

Traffic:
- Month 1: 100-200 visits
- Month 3: 500-1,000 visits
- Month 6: 2,000-5,000 visits
```

---

## Budget-Friendly SEO Strategy

**Free Tools Only:**

1. Google Search Console (rank tracking)
2. Google Analytics 4 (traffic analysis)
3. Google Pagespeed Insights (speed check)
4. Ubersuggest Free (keyword research)
5. AnswerThePublic (content ideas)
6. Google Sheets (manual tracking)

**Cost: $0/month**
**Time: 2-3 hours/week**
**Expected Results: 6 months to page 1**

---

**Last Updated:** November 22, 2025  
**Status:** Ready to use
